package com.xmg.richeditor_android.service;
...
//	private List<SearchSong.ResultEntity.SongsEntity> mp3Infos;	//存放Mp3Info对象的集合
	private BaseSong mp3Infos;	//播放的歌曲
...
//				}
	...
	public void onStart(Intent intent, int startId) {
		...
		try {
			...
			if (msg == MusicControl.PLAY_MSG) {	//直接播放音乐
                play(0);
            } else if (msg == MusicControl.PAUSE_MSG) {	//暂停
                pause();
            } else if (msg == MusicControl.STOP_MSG) {		//停止
                stop();
            } else if (msg == MusicControl.CONTINUE_MSG) {	//继续播放
                resume();
            } else if (msg == MusicControl.PRIVIOUS_MSG) {	//上一首
                previous();
            } else if (msg == MusicControl.NEXT_MSG) {		//下一首
                next();
            } else if (msg == MusicControl.PROGRESS_CHANGE) {	//进度更新
                currentTime = intent.getIntExtra("progress", -1);
                play(currentTime);
            } else if (msg == MusicControl.PLAYING_MSG) {
...
//		}
	...
	private void play(int currentTime) {
		try {
			...
			Toast.makeText(this,"未知歌曲",Toast.LENGTH_SHORT).show();
			e.printStackTrace();
		}
	}
